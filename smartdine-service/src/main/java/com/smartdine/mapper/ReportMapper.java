...
public interface ReportMapper {
    ...
    /*@Select("select DATE(delivery_time) as date from orders where status = 5 and delivery_time between #{begin} and #{end} group by DATE(delivery_time) order by date")
    ...
    @Select("select delivery_time from orders where status = 5 and delivery_time between #{begin} and #{nextDay} ")
    List<LocalDate> selectOne(LocalDate begin, LocalDate nextDay);

    @Select("select SUM(amount) from orders where status = 5 and delivery_time between #{begin} and #{nextDay} ")
